...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        if self.threads[from].state == ThreadState::Running {
            self.threads[from].state = ThreadState::Ready;
        }
        ...
        self.threads[to].state = ThreadState::Running;
        if let Some(entry) = self.threads[to].entry.take() {
            unsafe {
        ...
        unsafe {
            let from_ctx = &mut self.threads[from].ctx as *mut TaskContext;
            let to_ctx = &self.threads[to].ctx as *const TaskContext;
